...
The `Contexts Manager` fetches _basic information_ (health, permissions, resources counts, active resources counts) for all contexts, and Kubernetes objects details for the current context (and other contexts on request).

The `Contexts States Dispatcher` receives events from the Contexts manager for every change, and broadcast information to the webview through _RPC Channels_.
