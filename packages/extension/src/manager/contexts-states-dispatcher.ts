...
export class ContextsStatesDispatcher {
  ...
  init(): void {
    ...
    this.manager.onResourceUpdated(async event => {
      await this.updateResource(event.resourceName);
      await this.updateActiveResourcesCount();
    });
  }

  async updateHealthStates(): Promise<void> {
    await this.rpcExtension.fire(KUBERNETES_CONTEXTS_HEALTHS, undefined);
  }
  ...
  async updatePermissions(): Promise<void> {
    await this.rpcExtension.fire(KUBERNETES_CONTEXTS_PERMISSIONS, undefined);
  }
  ...
  async updateResourcesCount(): Promise<void> {
    await this.rpcExtension.fire(KUBERNETES_RESOURCES_COUNT, undefined);
  }

  async updateActiveResourcesCount(): Promise<void> {
    await this.rpcExtension.fire(ACTIVE_RESOURCES_COUNT, undefined);
  }
  ...
  async updateResource(resourceName: string): Promise<void> {
    await this.rpcExtension.fire(KUBERNETES_UPDATE_RESOURCE, { resourceName });
  }
