...
describe('port forwarding', () => {
  ...
  test('forward button should call', async () => {
    ...
    await vi.waitFor(() => {
      expect(remoteMocks.get(API_SYSTEM).getFreePort).toHaveBeenCalledWith(50_000);
      expect(remoteMocks.get(API_PORT_FORWARD).createPortForward).toHaveBeenCalledWith({
        forward: {
          localPort: 55_001,
          remotePort: 80,
        },
  ...
  test('open button should use window.openExternal with proper local port', async () => {
    ...
    await vi.waitFor(() => {
      expect(remoteMocks.get(API_SYSTEM).openExternal).toHaveBeenCalledWith('http://localhost:55076');
    });
  });
  ...
  test('remove button should use window.deleteKubernetesPortForward with proper local port', async () => {
    ...
    await vi.waitFor(() => {
      expect(remoteMocks.get(API_PORT_FORWARD).deletePortForward).toHaveBeenCalledWith(DUMMY_FORWARD_CONFIG);
    });
  });

  test('error from createKubernetesPortForward should be displayed', async () => {
    vi.mocked(remoteMocks.get(API_PORT_FORWARD).createPortForward).mockRejectedValue('Dummy error');
  ...
  test('error should be cleaned after deletion', async () => {
    // only reject ONCE
    vi.mocked(remoteMocks.get(API_PORT_FORWARD).deletePortForward).mockRejectedValueOnce('Dummy error');
  ...
  test('existing forward should display localhost port and copy', async () => {
    ...
    expect(remoteMocks.get(API_SYSTEM).clipboardWriteText).toHaveBeenCalledWith(expected);
  });
});
